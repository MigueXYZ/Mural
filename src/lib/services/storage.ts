...
export function validateCampaignSchema(raw: unknown): SchemaValidationResult {
  ...
  const data: CampaignData = {
    ...
    maps: obj.maps || [],
    fileSystem: Array.isArray(obj.fileSystem) ? obj.fileSystem : [],
    activeScopeFolderId: obj.activeScopeFolderId || 'all',
    customCalendar: obj.customCalendar,
    playlists: obj.playlists || [],
    atlas: obj.atlas || obj.maps || [],
    ...
    ? obj.nodes.map((node: any) => {
        ...
        return {
          ...
          data: {
            ...
            audioTrackId: node.data?.audioTrackId,
            audioPlaylistId: node.data?.audioPlaylistId,
          },
        };
      })
    : [];
