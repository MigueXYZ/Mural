...
{#if edge}
  ...
  >
    <div
      class="w-full max-w-lg max-h-[88vh] bg-zinc-900 border border-zinc-700/90 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150"
    >
      <!-- Modal Header (Fixed / Shrink-0) -->
      <div class="flex items-center justify-between border-b border-zinc-800 px-6 py-3.5 bg-zinc-950/80 shrink-0">
        <div>
      ...
      <!-- Modal Body (Scrollable with space-y-3.5) -->
      <div class="flex-1 overflow-y-auto p-5 space-y-3.5 text-xs">
        <!-- Relationship Category Selector -->
        <div>
          <label for="relation-category-selector" class="block text-[11px] font-medium text-zinc-400 mb-1">
            Tipo de Relação
          </label>
          <div id="relation-category-selector" class="grid grid-cols-3 gap-1.5">
            {#each relationCategories as item}
              {@const Icon = item.icon}
              <button
                type="button"
                onclick={() => handleSelectCategory(item.id)}
                class="px-2 py-1.5 rounded-xl border text-left flex items-center gap-1.5 transition cursor-pointer {relationType ===
                item.id
                  ? `${item.colorClass} ring-1 ring-amber-500/50`
                  : 'bg-zinc-950/60 border-zinc-800 text-zinc-400 hover:border-zinc-700'}"
              >
                <Icon class="w-3.5 h-3.5 shrink-0" />
                <span class="truncate font-semibold text-[11px]">{item.label}</span>
              </button>
            {/each}
          </div>
          ...
          {#if relationType === 'custom'}
            <div class="mt-2.5 space-y-2.5 p-3 rounded-xl bg-zinc-950/70 border border-zinc-800 animate-in fade-in duration-150">
              <ColorPicker
              ...
              <!-- Icon Picker -->
              <div>
                <span class="block text-[11px] font-medium text-zinc-400 mb-1">
                  Ícone Customizado
                </span>
                <div class="grid grid-cols-8 sm:grid-cols-10 gap-1 max-h-24 overflow-y-auto p-1 bg-zinc-900/60 rounded-xl border border-zinc-800/80">
                  {#each ICON_OPTIONS as opt}
                    ...
                    <button
                      ...
                      title={opt.name}
                      class="h-7 rounded-lg flex items-center justify-center transition cursor-pointer {selectedIcon === opt.id
        ...
        <div class="space-y-1.5">
          <div>
            <label for="edge-label-input" class="block text-[11px] font-medium text-zinc-300 mb-1">
              Rótulo da Ligação *
            </label>
            <input
              id="edge-label-input"
              ...
              bind:value={label}
              class="w-full px-3 py-1.5 bg-zinc-950 border border-zinc-800 rounded-lg text-xs text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-amber-500/60"
            />
          </div>
            ...
            <div class="flex items-center gap-1.5 flex-wrap">
              ...
              {#each activeCategory.defaultLabels as preset}
                <button
                  ...
                  class="px-2 py-0.5 rounded-md bg-zinc-950 border border-zinc-800 hover:border-zinc-700 text-[10px] text-zinc-400 hover:text-zinc-200 transition cursor-pointer"
                >
                  {preset}
                </button>
              {/each}
        ...
        <div class="grid grid-cols-2 gap-2.5">
          ...
          <div>
            <label for="path-type-selector" class="block text-[11px] font-medium text-zinc-400 mb-1">
              Formato da Linha
            </label>
            <div id="path-type-selector" class="flex items-center gap-1 bg-zinc-950 p-1 rounded-xl border border-zinc-800">
              {#each pathTypesList as p}
                ...
                <button
                  ...
                  class="flex-1 py-1 px-1.5 rounded-lg text-[11px] font-medium flex items-center justify-center gap-1 transition cursor-pointer {pathType ===
                  p.id
                ...
                >
                  <Icon class="w-3 h-3" />
                  <span class="truncate">{p.label.split(' ')[0]}</span>
                </button>
              {/each}
            </div>
          </div>
          ...
          <div>
            <label for="bidirectional-toggle-box" class="block text-[11px] font-medium text-zinc-400 mb-1">
              Direcionalidade
            </label>
            <button
              ...
              class="w-full h-[34px] px-3 rounded-xl border text-left flex items-center justify-between transition cursor-pointer {bidirectional
        ...
        <!-- Notes Textarea -->
        <div>
          <label for="edge-notes-input" class="block text-[11px] font-medium text-zinc-300 mb-1">
            Notas Secretas do Mestre sobre esta Relação
          </label>
          <textarea
            id="edge-notes-input"
            rows="2"
            placeholder="Ex: Como descobriram a ligação, DT de teste, implicações..."
            bind:value={notes}
            class="w-full px-3 py-1.5 bg-zinc-950 border border-zinc-800 rounded-lg text-xs text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-amber-500/60 resize-none leading-relaxed"
          ></textarea>
        </div>
    </div>

    <!-- Modal Footer Actions (Sticky / Shrink-0 at bottom) -->
    <div class="flex items-center justify-between px-6 py-3 border-t border-zinc-800 bg-zinc-950/90 shrink-0">
      <button
        type="button"
        onclick={handleDelete}
        class="px-3 py-1.5 rounded-lg text-rose-400 hover:bg-rose-950/40 text-xs font-medium flex items-center gap-1.5 transition cursor-pointer"
      >
      ...
      <div class="flex items-center gap-2">
        <button
          type="button"
          onclick={handleCancel}
          class="px-4 py-1.5 rounded-lg text-xs text-zinc-400 hover:bg-zinc-800 transition cursor-pointer"
        >
          Cancelar
        </button>
        <button
          type="button"
          onclick={handleSave}
          class="px-5 py-1.5 rounded-lg bg-amber-500/20 border border-amber-500/50 text-amber-300 hover:bg-amber-500/30 text-xs font-semibold flex items-center gap-1.5 transition active:scale-95 shadow-md cursor-pointer"
        >
